...
- `main.js` must stay under 5 MB, the Obsidian Sync Standard limit; the community review warns above it. Check the size after dependency changes. `esbuild.config.mjs`'s `slimDependencies` plugin leaves out lib dependencies the plugin never runs (all of `@effect/platform-node` except `NodeFileSystem`/`NodePath`, which also removes undici and its `node:sqlite` import, and MathJax's unused default font). Re-check it on every lib upgrade.
...
- The major version stays at 1. Use `1.X.0` (`npm version minor`) for major feature updates, and also for breaking changes such as raising `minAppVersion` or changing settings or stored data. Use `1.X.Y` (`npm version patch`) for minor updates and fixes. Never run `npm version major`.
- Once a version has been released (its Git tag exists), the next change starts a new version: run `npm version minor|patch --no-git-tag-version` on the feature branch, which updates all four files without committing or tagging, and add a new "(unreleased)" heading to the changelog. If a later change on the same unreleased version needs a bigger bump (a feature after a fix), raise it before release.
- To release, merge into `main`, replace "(unreleased)" in the changelog heading with the date, and tag that commit with the exact version and no `v` prefix (`git tag -a 1.2.0 -m 1.2.0`). Push `main` and the tag. The `Release` workflow (`.github/workflows/release.yml`) then builds, attests and publishes the release with the changelog section as notes. Never create or upload release assets by hand, or they lose their attestations.
...
- `ConfluencePlugin.authenticationClient` (in `main.ts`) + `src/desktopFetch.ts`: authenticated Confluence client. `desktopFetch` uses Node `https` to avoid CORS, rejects non-HTTPS URLs and refuses redirects so credentials can't be forwarded.
- `src/nodeRequest.ts`: the one Node HTTP(S) request used by `desktopFetch`, `krokiFetch` and the attachment downloader. It never follows redirects (a 3xx comes back with its `location`), allows plain HTTP only when asked (Kroki), and can cap the body size. Each caller applies its own redirect policy on top.
- `src/electronRenderers.ts`: Mermaid and LaTeX rendering in hidden Electron windows (`@electron/remote`), adapted from upstream `mermaid-electron-renderer`. Mermaid is Obsidian's own (`loadMermaid()`), not a bundled copy, which keeps `main.js` small; its configuration is restored after each render because Obsidian's notes share it.
- `src/mermaidStyles.ts`: theme CSS, snippets and Mermaid config for diagram rendering, loaded once per publish. `src/publishSelection.ts`: the enable/disable publishing rules. Shared helpers: `src/errors.ts`, `src/paths.ts`, `src/fences.ts` (fenced-code tracking for Markdown rewrites), and `src/frontmatterKeys.ts` (`connie-*` keys, taken from the lib's per-page config).
...
- `src/imageEmbeds.ts` + `src/imageSize.ts`: a publish-time source transform (after callouts in `main.ts` `sourceTransformer`) that rewrites local image embeds to an explicit `WIDTHxHEIGHT`, which the lib turns into the media node's size. Images wider than `maxImageWidth` are scaled down, and a width-only size gets the matching height (the lib would keep the full height). Sizes come from the file header (`imageSize`, with EXIF rotation). Notes are never changed.
...
- `src/sync/`: pull (Confluence → Obsidian) and the pre-publish check, modeled on git pull/push.
  - `syncState.ts` stores a **base snapshot** per page (version, title, and the body converted to Markdown) in `<plugin dir>/sync/<pageId>.json`, recorded after every successful publish (`PullService.recordPublished`, called from `main.ts`) and pull.
  ...
  - `pageLinks.ts` rewrites links to site pages that have notes into wikilinks (`[[Note|text]]`, `\|` in table rows), after the `publishesAs` check, since wikilinks don't parse back to the same ADF. `pull.ts` `convertPage` = `adfToMergeMarkdown` + `rewritePageLinks`; use it for every base, remote and import conversion, with a resolver built from linked notes (and planned imports). Snapshots record `unresolvedLinks`; a pull re-converts a note when any of them now has a note. Imports are planned for the whole tree before any note is written, so imported pages can link to each other.
  - `media.ts` `MediaSync` makes page attachments local files: `ensure(adf)` matches each media file ID to its attachment, reuses files the publisher uploaded from this vault (title `<md5>-<name>`), otherwise downloads into `settings.imageFolder`, and stores fileId → path in `sync/media.json`. `resolver()` feeds `adfToMergeMarkdown`, which emits `![[file]]` for `mediaSingle`/`mediaGroup` without the round-trip check. Publishing (`recordPublished`) never downloads.
  - `attachmentDownload.ts`: Confluence answers downloads with a redirect to the media service, and the lib transport refuses redirects. The downloader wraps the transport's fetch for `/child/attachment/<id>/download` only, captures the redirect, and fetches it **without credentials**, over HTTPS, only from `*.atlassian.com`/`*.atlassian.net`, at most 100 MB. Keep those guarantees and their tests.
  ...
  - `fingerprint.ts`: "Publish changes" sends only notes whose fingerprint (text, path, embedded files' mtime/size, linked notes' page IDs, publish-relevant settings) differs from `SyncBase.localFingerprint`. Publishing records it for every uploaded note; a pull keeps it only if the note was in sync before and the merge had no conflicts, and imports start in sync. Notes with Dataview queries never get one. Bump `FINGERPRINT_FORMAT` when the inputs change.
  - `partialPublish.ts`: the lib publishes one note or all, and builds the page tree from the notes it's given (root = shared folder path; a folder without its folder note gets a generated page). `notesForPartialPublish` adds the folder notes on the way to each changed note, and a note that keeps the root, mirroring the lib's `TreeLocal.ts`; `partialWorkspace` narrows the lib's workspace to that set. **Re-check these rules on every lib upgrade**, or pages can move or folder pages be overwritten. `main.ts` runs the publish gate on the whole set, turns off `forceOverwrite` and `orderPages` for partial runs, and hides the folder notes' results.
...
- `src/ConfluenceSettingTab.ts` (with the OAuth browser sign-in section in `src/BrowserLoginSettings.ts`) describes each setting once as a `SettingRow` (`src/settingRows.ts`): `getSettingDefinitions()` turns them into `render` definitions on Obsidian 1.13+, which makes them searchable, and `display()` renders the same rows on older versions. After a change that adds or removes rows, call `redisplay()` (`update()` on 1.13+), never `display()`. Other UI: `src/CompletedModal.ts` and `src/ConfluencePerPageForm.ts`. The modals build their DOM with `createEl` and empty `contentEl` in `onClose`. The per-page form is generated from the lib's `ConfluencePageConfig.conniePerPageConfig`. Styles live in `styles.css` and use Obsidian CSS variables.

## Obsidian plugin rules

All new and changed code must follow these rules. They come from Obsidian's Plugin guidelines, Submission requirements, Developer policies, the plugin self-critique checklist, and the lifecycle, load-time and SecretStorage guides. WebFetch on docs.obsidian.md often returns "Not Found". To re-check a rule, fetch the source markdown from `https://raw.githubusercontent.com/obsidianmd/obsidian-developer-docs/main/en/<path>.md`, for example `Plugins/Releasing/Plugin guidelines` or `Community directory/Developer policies` (URL-encode spaces). Check the `@since` tags in `obsidian.d.ts` before using an API, and raise `minAppVersion` in `manifest.json` when you depend on something newer.
...
- `mermaidStyles.ts` `readConfigCss` uses `vault.adapter`, because the Vault API doesn't index the config directory. Path segments come from app config and are checked before joining.
- `mermaidStyles.ts` `getVaultConfig` uses the undocumented `vault.getConfig` to find the active theme and snippets, behind a typed optional accessor.
...
- `eslint.config.mjs` turns off `no-nodejs-modules` (desktop-only). It also relaxes the `no-unsafe-*` rules in tests. The OAuth modules use `window.setTimeout`, so their tests stub `window` with Node's timers.
...
- `ConfluenceSettingTab` keeps `display()` as a fallback for Obsidian before 1.13.0 and calls the pre-1.13 `setDynamicTooltip` through a typed cast. When `minAppVersion` reaches 1.13.0, drop both and `renderSections`.
- The bundle contains code the plugin never runs, which the community review reports: Node `fs` (the lib's `NodeFileSystem`, for its CLI; the plugin provides a vault-based `FileSystem`) and `localStorage` (Atlassian's feature-flag client inside the lib). Electron IPC (`@electron/remote`) is real and needed for rendering. The README's Disclosures section covers all three.
