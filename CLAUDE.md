...
- To release, merge into `main`, replace "(unreleased)" in the changelog heading with the date, and tag that commit with the exact version and no `v` prefix (`git tag -a 1.2.0 -m 1.2.0`). Push `main` and the tag, then create the GitHub release with `gh release create VERSION main.js manifest.json styles.css` from a fresh `npm run build`.
