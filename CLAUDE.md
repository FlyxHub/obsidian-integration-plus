...
- `src/sync/`: pull (Confluence → Obsidian) and the pre-publish check, modeled on git pull/push.
  ...
  - `media.ts` `MediaSync` makes page attachments local files: `ensure(adf)` matches each media file ID to its attachment, reuses files the publisher uploaded from this vault (title `<md5>-<name>`), otherwise downloads into `settings.imageFolder`, and stores fileId → path in `sync/media.json`. `resolver()` feeds `adfToMergeMarkdown`, which emits `![[file]]` for `mediaSingle`/`mediaGroup` without the round-trip check. Publishing (`recordPublishedBases`) never downloads.
