...
- **Vault access:** To find the notes to publish, match Confluence pages to notes, and turn page links into wikilinks, the plugin lists the files and notes in your vault and reads their frontmatter. It reads and changes notes and attachments only through Obsidian's vault API.
- **Electron windows:** To render Mermaid diagrams and LaTeX equations as images, the plugin opens hidden Electron windows through the `@electron/remote` module, which communicates with Obsidian's main process over Electron IPC. Each window loads only the diagram or equation that it renders, and closes when rendering finishes.
- **Clipboard:** In device code sign-in, the **Copy code** button copies the device code to the clipboard. The plugin never reads the clipboard.
- **Bundled library code:** The Confluence library that the plugin bundles includes Node.js file system code for its command-line tool. The plugin gives the library Obsidian's vault API for file access instead. Atlassian's document format library, which is also bundled, includes a feature-flag client that can use browser `localStorage`. The plugin itself doesn't use `localStorage` or `sessionStorage`.
