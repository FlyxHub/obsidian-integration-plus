...
export class AtlassianOAuth {
	...
	private async request(url: string, init: RequestInit, signal?: AbortSignal) {
		...
		try {
			...
			throw new OAuthNetworkError("Cannot reach Atlassian. Check your connection and try again.");
		}
		let data: unknown;
		try {
	...
	private error(data: Record<string, unknown>, device = false): Error {
		...
		if (data["error"] === "invalid_grant")
			return new Error("Session expired or authorization was rejected. Reconnect to Atlassian.");
	...
	async browser(
		...
		const code = await this.receiveCode(callbackUrl, state, signal, () => open(authorization.href));
	...
	async sites(accessToken: string, signal: AbortSignal): Promise<OAuthSite[]> {
		...
		const sites = (response.data as unknown[])
			.map(parseSite)
			.filter((site): site is OAuthSite => site !== undefined);
		if (!sites.length)
			throw new Error("No Confluence site was granted. Check the app's scopes and reconnect.");
		return sites;
	}
}

/** Keep only well-formed Atlassian Cloud sites that granted Confluence read access. */
function parseSite(entry: unknown): OAuthSite | undefined {
	if (!entry || typeof entry !== "object") return undefined;
	const { id, url, name, scopes } = entry as Record<string, unknown>;
	if (
		typeof id !== "string" ||
		...
		!scopes.includes("read:page:confluence")
	)
		return undefined;
