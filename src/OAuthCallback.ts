...
export function oauthCallbackUrl(value: string): URL {
	...
	if (
		...
		throw new Error("Use http://127.0.0.1:PORT/callback and register the same URL with Atlassian.");
	return url;
}
	...
	return new Promise((resolve, reject) => {
		...
		const server = createServer({ maxHeaderSize: 16384 }, (request, response) => {
			...
			if (
				...
				reply(400, "This login response is invalid. Continue the login started in Obsidian.");
				return;
			}
